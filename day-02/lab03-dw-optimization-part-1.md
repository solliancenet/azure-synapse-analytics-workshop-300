...
5. Run the following (more complex) statement (where `SUFFIX` is your **student ID**):
...
6. Check the structure of the `Sale_Heap` table, by right clicking on it in the `Data` hub and selecting `New SQL script` and then `CREATE`. Take a look at the script used to create the table:
...
7. Run the same script as the one you've run at step 2, but this time with the `EXPLAIN WITH_RECOMMENDATIONS` line before it (where `SUFFIX` is your **student ID**):
...
8. Besides the `EXPLAIN` statement, you can also understand the plan details using the `sys.dm_pdw_request_steps` DMV.
...
9. With the query id (`QID473552` in this case) you can now investigate the individual steps of the query:
...
10. Get more details on the problematic step using the following SQL statement:
...
11. Finally, you can use the following SQL statement to investigate data movement on the distributed databases:
...
1. Create an improved version of the table using CTAS (Create Table As Select) (where `SUFFIX` is your **student ID**):
    ...
    The query will take about 2-4 minutes to complete.
...
CREATE TABLE [wwi_perf].[Sale_Partition02_SUFFIX]
WITH
(
    DISTRIBUTION = HASH ( [CustomerId] ),
    CLUSTERED COLUMNSTORE INDEX,
...
> **Note** These queries will take up to 7-9 minutes to run.
