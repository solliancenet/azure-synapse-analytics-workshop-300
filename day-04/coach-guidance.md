...
def findnth(haystack, needle, n):
    parts= haystack.split(needle, n+1)    ##print(parts)
    if len(parts)<=n+1:
        return -1
    return len(haystack)-len(parts[-1])-len(needle)
    ### TEST Texttext = '00000317434967201,20170502,10.52700000000000084998674765,5,45,7296,ad399119-7a32-4c64-89d6-380b42e83634,999780,2742,3,27.110000000000000000,89.46300000000000722355508742,20170502,27.25800000000000220090612402,19,34,1611'
    ### Uncomment and change to read from ADLS Gen2 #f = open("sale-20170502.csv", "r")
    #text = f.read()needle = ","
    g = findnth(text,needle,10)
    length = len(text)
    cond = True
    multplier = 10i = 1
    ...
    while cond:
        char = i * multplier +1
        mc = findnth(text,needle,char)
        ## Better solution is to subsrting each line and write to a file -- much better memory efficiency
        text = text[:mc] + '|' + text[mc+1:]
        #print(char)
        i = i+1
